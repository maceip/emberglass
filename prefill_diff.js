...
window.run = async () => {
  ...
  const rt = new QwenWGPU(dev, cfg); await rt.build('/model'); console.log('VWG built; maxPrefillT=' + rt.maxPrefillT + ' maxCtx=' + rt.maxCtx);
  ...
  for (const L of [16, 17, 256, 257, 512, 1024]) {
    ...
    allOk = allOk && ok;
    console.log(`VWG L=${String(L).padStart(4)}  argmax ${seqGen[0]}==${batGen[0]}  gen ${JSON.stringify(seqGen)===JSON.stringify(batGen)?'match':'DIFFER'}  logitΔ=${maxAbsDiff(Lseq,Lbat).toFixed(3)}  ${ok?'PASS':'FAIL'}`);
  }
  for (const L of [4096, 8192]) {
    const t0 = performance.now(); rt.prefillBatch(tile(L)); const lg = await readLogits(); const ms = performance.now() - t0;
    const ok = finite(lg) && argmax(lg) < cfg.vocabSize;
    allOk = allOk && ok;
    console.log(`VWG smoke L=${L}  prefill=${ms.toFixed(0)}ms  finite=${finite(lg)}  argmax=${argmax(lg)}  ${ok?'PASS':'FAIL'}`);
  }
