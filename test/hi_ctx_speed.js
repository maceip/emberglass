...
window.run = async () => {
  ...
  const rt = new QwenWGPU(dev, QWEN25_3B); await rt.build('/model'); console.log('VWG built maxCtx='+rt.maxCtx);
  ...
  for (const startCtx of [2000, 4000, 6000, 7800]) {
    ...
    let pos = startCtx, nxt = await rt.argmaxLogits();
    await dev.queue.onSubmittedWorkDone();
    const N = 40, t0 = performance.now();
    for (let i=0;i<N;i++){ const b = await rt.decodeBatch(pos, Math.min(rt.MAXBATCH, N-(i*rt.MAXBATCH<N?0:0))); pos += b.length; i += b.length-1; }
    const dt=(performance.now()-t0)/1000, toks=pos-startCtx, tps=toks/dt;
    console.log('VWG ctx~'+startCtx+': '+tps.toFixed(1)+' tok/s ('+toks+' tok in '+dt.toFixed(2)+'s) '+(tps>=20?'>=20 OK':'BELOW 20'));
  }
