...
- Kernel source now lives in `src/qwgpu/templates/**/*.wgsl.jinja`.
- `src/qwgpu/kernels.js` and `src/qwgpu/backward_kernels.js` are generated by `scripts/generate_kernel_modules.mjs`.
- `npm run kernels:check` fails if generated kernel modules drift from templates.
- `build()` issues ~53 synchronous `createShaderModule` + `createComputePipeline` calls.
- Many near-duplicate templates remain (F16 variants, paged, W4A8, prefill styles).
...
- Build-time Jinja template source-of-truth for forward and backward kernels.
- Generated kernel modules with one ASCII header per generated file and a clear "edit templates" notice.
- `kernels:generate`, `kernels:check`, and esbuild integration.

### Remaining High-Leverage Tactics

1. Switch to `createComputePipelineAsync` + parallel await during build (biggest responsiveness win).
2. Build-time WGSL minification / comment+whitespace stripping in the generator (shrinks rendered source in the bundle and what the WGSL parser sees).
3. Consolidate F16 variants into canonical Jinja templates instead of maintaining duplicate template files.
